...
let setup () =
  ...
  (Grid.init (), Input_box.init (), Calculator.init_env (), font)

let rec loop (grid: Grid.t) (input_box: Input_box.t) (env : Calculator.env_t) font =
  ...
  | false -> 
    ...
    loop grid input_box env font

let () = 
  let grid, input_box, env, font = setup () in
  loop grid input_box env font
